...
var widthOfNumbers = 62;//the width in px of the numbers on the left
var buttonLink = "http://codecanyon.net/user/dxc/portfolio?ref=dxc";
var endDate = new Date(2014, 8, 22, 22, 30);//year, month, day, hour, minute //IMPORTANT!!! January = 0, February = 1, and so on. 
var timeZone = "America/New_York";//get timezone from this list: http://php.net/manual/en/timezones.php

//set the text
var buttonText = "Donate Now!";//text that goes on the button
var daysLeftText = " days left, ";
var hoursLeftText = " hours left, ";
var minutesLeftText = " minutes left, ";
var timesUpText = "Time's up!";
var fundedSuffix = "% funded";
...
function createGraphics(){
	...
	//determine % complete
	var percComplete = Math.floor(currentAmount/goalAmount * 100);
	...
	if(minutesLeft < 0){
		//time is up
		timeLeftString = timesUpText;
	}else if(minutesLeft < 120){
		//less than an hour
		timeLeftString = Math.floor(minutesLeft) + minutesLeftText;
	}else if(minutesLeft < 2880){
		//less than a day
		timeLeftString = Math.floor(minutesLeft/60) + hoursLeftText;
	}else{
		//more than a day
		timeLeftString = Math.floor(minutesLeft/1440) + daysLeftText;
	}
	
	//add the html
	$("#goal-thermometer").append(
		"<div id='therm-holder'>" +
	...
	if(buttonText != ""){
		$("#goal-thermometer").append(
			"<div id='therm-button'><a href='" + buttonLink + "'>" + buttonText + "</a></div>"
		);
	};
	
	if(timeZone != ""){
		$("#goal-thermometer").append(
			"<div id='time-remaining'>" + timeLeftString + " " + percComplete + fundedSuffix + "</div>"
		);
	};
	...
	$("#therm-holder").css("height",  heightOfBody + thermTopHeight + thermBottomHeight);
...
function getTZOutput(getTZName) {
	
	var dNow = new Date();
	var thisOffset = -dNow.getTimezoneOffset();//figure out user's timezone
	//console.log(thisOffset);
	
   	$.ajax({
   		type: "POST",
	    url:'php/getTimezoneTime.php',
	    data: {thisTz:getTZName},
	    dataType: "json",
	    success: function (response) {
	    	var tzOffset = response/60;
	    	
	    	timezoneDifference = tzOffset - thisOffset;
	    	
	    	
	    	var MS_PER_MINUTE = 60000;
	    	var adjustedEndDate = new Date(endDate - timezoneDifference * MS_PER_MINUTE);
	    	
	    	minutesLeft = (adjustedEndDate.getTime() - dNow.getTime())/1000/60;
